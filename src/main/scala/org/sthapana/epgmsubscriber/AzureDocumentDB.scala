...
class AzureDocumentDB(host: String, password: String, db: String, collection: String) {
  ...
  private def insertNewRecord(logRecord: Record, masterDoc: Document) = {
    val rcordCount: Int = Integer.parseInt(masterDoc.get("recordnumber").toString) + 1
    ...
    Validator.apply(combinedRecord("statecode"), combinedRecord, masterDoc) match {
      case Left(x) => {
        ...
        val upd = new UpdateEntity(logRecord("statecode"), logRecord("districtcode"),
          ...
          logRecord("whounderweight"), masterDoc.get("sex").toString, currentAge.toString,
  ...
  private def insertFirstRecord(record: Record) = {
    ...
    def getRecordFromMasterData(): Record = {
      ...
      val results = documentClient.queryDocuments(col.getSelfLink,
        "SELECT * FROM myCollection where myCollection.doctype=\"child\" and myCollection.aanganwadicode=\"" + logRecord("aanganwadicode") + "\" and myCollection.childcode=\"" + logRecord("childcode") + "\"",
        null).getQueryIterable().toList;
      if (!results.isEmpty) {
