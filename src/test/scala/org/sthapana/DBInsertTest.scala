...
class DBInsertTest {
  ...
  def testInsertAzureDocDB(): Unit ={
    ...
    documentClient.createDocument(col.getSelfLink,entityDocument,null,false)*/
     val updateEntityWithOutMonthChange: UpdateEntity = UpdateEntity("27", "", "", "", "", "2", "M", "5", "03", "20")
     new ProcessingEngine().updateDB(updateEntityWithOutMonthChange)
    //then
  }

  @Test
  def itShouldInsertDashboardRecord(): Unit = {
    //given
    ...
    documentClient.createDocument(col.getSelfLink,
